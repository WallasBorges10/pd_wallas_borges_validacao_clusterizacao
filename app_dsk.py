...
def load_data(uploaded_file=None, sample_size=None, random_state=42):
    """
    Carrega dados de um arquivo CSV ou usa dados de exemplo.
    
    Args:
        uploaded_file: Arquivo carregado pelo usuário
        sample_size: Tamanho da amostra (None para todos os dados)
    ...
    if uploaded_file is not None:
        try:
            ...
            st.info("Carregando dataset de exemplo...")
...
def create_sample_data():
    ...
    # Dataset de exemplo baseado na estrutura do notebook
    n_samples = 5000
    ...
    data = {
        ...
        'data': pd.date_range('2023-01-01', periods=n_samples, freq='H').strftime('%Y-%m-%d'),
        'horario': pd.date_range('2023-01-01', periods=n_samples, freq='H').strftime('%H:%M:%S'),
        'dia_semana': np.random.choice(['segunda-feira', 'terça-feira', 'quarta-feira', 
                                        'quinta-feira', 'sexta-feira', 'sábado', 'domingo'], n_samples),
        'uf': np.random.choice(['SP', 'RJ', 'MG', 'RS', 'PR', 'SC', 'BA', 'PE'], n_samples, p=[0.3, 0.15, 0.15, 0.1, 0.1, 0.05, 0.1, 0.05]),
        'municipio': np.random.choice(['São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 
                                       'Porto Alegre', 'Curitiba', 'Florianópolis', 
                                       'Salvador', 'Recife'], n_samples),
        'causa_acidente': np.random.choice(['Reação tardia', 'Excesso de velocidade', 
                                            'Desobediência à sinalização', 'Ingestão de álcool',
                                            'Falta de atenção', 'Condições adversas'], n_samples),
        'tipo_acidente': np.random.choice(['Colisão traseira', 'Colisão frontal', 
                                           'Atropelamento', 'Saída de via', 
                                           'Queda de motocicleta', 'Colisão lateral'], n_samples),
        'condicao_metereologica': np.random.choice(['Céu claro', 'Chuva', 'Nublado', 
                                                    'Garoa/Chuvisco', 'Nevoeiro/Neblina'], n_samples),
        'tipo_pista': np.random.choice(['Simples', 'Dupla', 'Múltipla'], n_samples, p=[0.4, 0.4, 0.2]),
        'tracado_via': np.random.choice(['Reta', 'Curva', 'Cruzamento', 'Rotatória'], n_samples),
        'condicao_pista': np.random.choice(['Seca', 'Molhada', 'Danificada'], n_samples),
        'conservacao_pista': np.random.choice(['Boa', 'Regular', 'Ruim'], n_samples, p=[0.6, 0.3, 0.1]),
        'latitude': np.random.uniform(-33, -5, n_samples),
        'longitude': np.random.uniform(-75, -35, n_samples),
        'br': np.random.choice([101, 116, 153, 290, 381, 50, 40, 262], n_samples),
        'km': np.random.uniform(0, 300, n_samples),
        'mortos': np.random.poisson(0.1, n_samples),
        'feridos_graves': np.random.poisson(0.3, n_samples),
        'feridos': np.random.poisson(1.2, n_samples),
        'ilesos': np.random.poisson(2.0, n_samples),
        'ignorados': np.random.poisson(0.1, n_samples),
        'veiculos': np.random.poisson(1.5, n_samples)
    }
    
    df = pd.DataFrame(data)
    
    # Adicionar algumas correlações
    df.loc[df['condicao_metereologica'] == 'Chuva', 'condicao_pista'] = 'Molhada'
    df.loc[df['tracado_via'] == 'Curva', 'tipo_acidente'] = np.random.choice(['Saída de via', 'Colisão lateral'], 
                                                                              len(df[df['tracado_via'] == 'Curva']))
...
def data_loading_section():
    ...
    if load_option == "Upload de arquivo":
        uploaded_file = st.file_uploader(
            ...
            type=['csv', 'txt'],
            help="Formatos suportados: CSV com separador vírgula ou ponto-e-vírgula"
        )
    
    # Opções avançadas na sidebar
    ...
    # Carregar dados
    if load_option == "Upload de arquivo" and uploaded_file is not None:
        df = load_data(uploaded_file=uploaded_file, sample_size=sample_size, random_state=random_state)
    else:
        df = load_data(sample_size=sample_size, random_state=random_state)
...
def eda_geographic_analysis_advanced(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        if 'br' in df_geo.columns and 'km' in df_geo.columns:
            ...
            if len(df_geo) > 0:
                df_geo['trecho'] = df_geo['km'].apply(lambda x: f"{int(float(x)//10)*10}-{int(float(x)//10)*10+10}km")
...
# Integração com sua função principal
def integrate_with_main_app():
    """
    Exemplo de como integrar as novas funções na sua aplicação principal.
    """
    # Na sua função main, substitua a chamada à eda_section por:
    
    # Substituir:
    # elif page == "🔍 Análise Exploratória":
    #     eda_section(st.session_state.df)
...
def main():
    ...
    st.markdown("""
    Esta aplicação realiza análise exploratória e clusterização de dados de acidentes de trânsito.
    Convertida de notebook Jupyter para aplicação Streamlit interativa.
    """)
    ...
    - **Desenvolvido por**: Cientista de Dados Sênior
